// apps/lead_dedupe/lead_dedupe/public/js/crm_lead_form.js
frappe.ui.form.on('CRM Lead', {
  async before_save(frm) {
    // Proactively clear a stale "Duplicate Of" so server link validation can't fail
    if (frm.doc.sr_duplicate_of) {
      try {
        await frappe.call('lead_dedupe.api.dup_fix.fix_duplicate_of_if_stale', {
          name: frm.doc.name || ''
        });
      } catch (e) {
    ...
    if (!frm.is_new() && cint(frm.doc.sr_dup_hit_count) > 0) {
      frm.add_custom_button('View & Merge Duplicates', async () => {
        if (!window.openCRMLeadDuplicatesDialog) {
          await new Promise(resolve => frappe.require('/assets/lead_dedupe/js/crm_lead_modal.js', resolve));
        }
