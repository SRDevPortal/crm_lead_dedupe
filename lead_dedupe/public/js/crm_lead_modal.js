// apps/lead_dedupe/lead_dedupe/public/js/crm_lead_modal.js
...
async function openCRMLeadDuplicatesDialog({ primary_name }) {
  try {
    ...
    const { message: rows = [] } = await frappe.call(
      'lead_dedupe.api.crm_lead_duplicates.get_duplicates_for_crm_lead',
    ...
    d.$body.find('[data-action="merge"]').on('click', async () => {
      ...
      await frappe.call('lead_dedupe.api.crm_lead_merge.merge_crm_leads', {
